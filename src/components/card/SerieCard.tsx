    ...
    return (
        <div className='w-full md:max-w-[18.75rem] hover:-translate-y-4 transition-all ease-in-out duration-300 mb-12'>
            <div className='w-full max-h-[16.875rem] h-full rounded-md overflow-hidden'>
                {
                    serie.backdrop_path ?
                        <img src={`${API.baseImageURL}${POSTER_SIZE}${serie.backdrop_path}` ?? DefaultMovieImage} alt="imagem do filme" className='object-cover h-full' />
                    :
                        <img src={DefaultMovieImage} alt="imagem do filme" className='object-cover h-full' />
                }
            </div>
            <div className='flex mt-4 gap-2'>
                <p className='flex-1 font-medium font-poppins text-lg truncate'>{serie.name ? serie.name : 'sem título'}</p>
                <div className='flex gap-3 items-center'>
                    <p className='font-light font-inter text-lg'>{serie.vote_average ? serie.vote_average.toFixed(2) : '?'}</p>
            ...
            <div className='flex justify-between mt-4'>
                ...
                <div className='flex gap-3'>
                    ...
                    <p className='font-inter'>{serie.first_air_date ? serie.first_air_date.split('-')[0] : '????'}</p>
                </div>
            </div>
        </div> 
    )
